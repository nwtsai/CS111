...
void processInput()
{
  ...
  ssize_t num_bytes; 
  num_bytes = SSL_read(sslDescriptor, user_input, 1024);
  if(num_bytes < 0)
  {
    if(log_flag == 1)
    {
  ...
  if(strncmp(user_input, "OFF\n", 4) == 0)
  {
    ...
    if(socket_flag == 1)
    {
      int buffer_length;
      char text[18];
      buffer_length = sprintf(text, "%s SHUTDOWN\n", time_stamp);
      if(buffer_length < 0)
      {
        fprintf(stderr, "Error: Failed to place SHUTDOWN message into text buffer\n");
      }
      int SSLWriteStatus = SSL_write(sslDescriptor, text, buffer_length);
      if(SSLWriteStatus < 0)
      {
...
{  
  ...
  if(temperature < 10)
  {
    ...
    if(socket_flag == 1)
    {
      char text[14];
      int buffer_length;
      buffer_length = sprintf(text, "%s 0%.1f\n", time_stamp, temperature);
      int SSLWriteStatus = SSL_write(sslDescriptor, text, buffer_length);
      if(SSLWriteStatus < 0)
      {
        fprintf(stderr, "Error: SSL write failed\n");
  ...
  else
  {
    ...
    if(socket_flag == 1)
    {
      char text[14];
      int buffer_length;
      buffer_length = sprintf(text, "%s %.1f\n", time_stamp, temperature);
      int SSLWriteStatus = SSL_write(sslDescriptor, text, buffer_length);
      if(SSLWriteStatus < 0)
      {
        fprintf(stderr, "Error: SSL write failed\n");
...
void closeAll(int exit_code)
{
  ...
  if(sslDescriptor != NULL)
  {
    SSL_shutdown(sslDescriptor);
    SSL_free(sslDescriptor);    
  }
...
int main(int argc, char** argv) 
{
  ...
  if(id != NULL && host != NULL && port_num != -1)
  {
    ...
    sslDescriptor = SSL_new(sslContext);
    ...
    if(connectStatus < 0)
    { 
      fprintf(stderr, "Error: Host does not exist or invalid port number\n");
      closeAll(1);
    }
    else 
    {
    ...
    SSL_set_fd(sslDescriptor, client_socket_fd);
    SSL_connect(sslDescriptor);

    if(socket_flag == 1)
    {
      int buffer_length;
      char text[14];
      buffer_length = sprintf(text, "ID=%s\n", id);
      if(buffer_length < 0)
      {
        fprintf(stderr, "Error: Failed to place id into text buffer\n");
      }
      int SSLWriteStatus = SSL_write(sslDescriptor, text, buffer_length);
      if(SSLWriteStatus < 0)
      {
  ...
  while(should_run)
  {
    ...
    if(pollStatus > 0)
    {
      // If the server has sent input that is ready to be read
